...
def file_search(dirlists, work_dir):
		...
		if os.path.exists(work_path):
			...
			if len(work_files) != 0:
				...
				for j in range(0, len(work_files)):
					...
					try:
						temp_df['Ship Status'] = temp_df['Ship Status'].fillna(0).astype(int)
					except Exception as ex:
						print('error occurred -----------------> ', ex)	
						temp_df['Ship Status'] = temp_df['Ship Status'].str.replace(',','').fillna(0).astype(int)	
	...
	print(df.columns)
	df = df.reset_index().rename(columns={'Ship Status':'Code Match'})
...
if __name__ == "__main__":
	...
	target_dir = "\\data\\2019\\NIPA_Support"
